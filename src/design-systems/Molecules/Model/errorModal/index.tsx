...
import { ErrorIcon } from 'design-systems/Atoms/Icons'
interface AuthModalProps {
  isCheck: boolean
  setCheck: (value: boolean) => void
  setDropdownVisibilityClaim: (value: boolean) => void
}
export const ErrorModal: React.FC<AuthModalProps> = ({ isCheck, setCheck, setDropdownVisibilityClaim }) => {
                ...
                <Dialog.Panel className="relative max-w-[400px] transform overflow-hidden rounded-[16px] bg-white p-5 text-center font-Rubik shadow-xl transition-all dark:bg-black18 sm:my-8 sm:w-full sm:p-10">
                  <div className=" items-center justify-center">
                    <div className="fixed left-0 top-0 mt-[69px] block w-full   ">
                      ...
                      <Typography
                        className={`mb-4 mt-2 justify-center text-center text-[27px] font-bold ${
                      ...
                      <div className="mt-3.5 flex w-full justify-center text-center">
                        <div className="w-[20rem]">
                          Your wallet is not an owner of this collection. Try to log in with correct wallet. If the
                          problem persist please Contact support
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="mt-[15rem] flex justify-center gap-x-10">
                    <Button
                      className="!h-[45px] !w-[57%] rounded-full font-semibold"
                      hover={true}
                      outlineBorder={false}
                      text={'Clone and continue'}
                      onClick={() => {
                        setCheck(false)
